...
final class ApiDocumentationRegistry {
    ...
    private void registerAuth() {
        ...
        add("AuthController", "logout", "Xác thực", "Đăng xuất",
                ...
                "204", "Đăng xuất thành công, không có response body", null, "LOGOUT_REQUEST", ResponseKind.VOID);
        addPublic("AuthController", "changePassword", "Xác thực", "Đổi mật khẩu",
                "Đổi mật khẩu khi username và mật khẩu hiện tại hợp lệ.",
                "200", "Mật khẩu đã được thay đổi", "STRING_SUCCESS", "CHANGE_PASSWORD_REQUEST", ResponseKind.TEXT, Set.of());
        addPublic("AuthController", "forgotPassword", "Xác thực", "Yêu cầu đặt lại mật khẩu",
                ...
                "200", "Đã tiếp nhận yêu cầu", "STRING_SUCCESS", "FORGOT_PASSWORD_REQUEST", ResponseKind.TEXT, Set.of());
        addPublic("AuthController", "resetPassword", "Xác thực", "Đặt lại mật khẩu",
                ...
                "200", "Mật khẩu đã được đặt lại", "STRING_SUCCESS", "RESET_PASSWORD_REQUEST", ResponseKind.TEXT, Set.of());
    }
    ...
    private void registerDoctors() {
        ...
        add("DoctorController", "editDoctor", "Bác sĩ", "Cập nhật bác sĩ",
                ...
                "200", "Thông tin bác sĩ sau cập nhật", "DOCTOR", "EDIT_DOCTOR_REQUEST", ResponseKind.JSON);
    ...
    private void registerPatients() {
        add("PatientController", "createPatient", "Bệnh nhân", "Tạo bệnh nhân",
                ...
                "201", "Hồ sơ bệnh nhân đã tạo", "PATIENT", "CREATE_PATIENT_REQUEST", ResponseKind.JSON);
        ...
        add("PatientController", "editPatient", "Bệnh nhân", "Cập nhật bệnh nhân",
                ...
                "200", "Hồ sơ bệnh nhân sau cập nhật", "PATIENT", "EDIT_PATIENT_REQUEST", ResponseKind.JSON);
    ...
    private void registerExaminations() {
        add("ExaminationController", "getAllExaminations", "Ca khám", "Lấy toàn bộ ca khám",
                "Trưởng khoa xem danh sách ca khám toàn khoa có phân trang.", "200", "Trang ca khám", "EXAMINATION_PAGE", null, ResponseKind.JSON);
        add("ExaminationController", "getExaminationsSortedByStudyDate", "Ca khám", "Sắp xếp theo ngày chụp",
                "Sắp xếp ca khám trong phạm vi được phép theo studyDate tăng hoặc giảm.", "200", "Trang ca khám đã sắp xếp", "EXAMINATION_PAGE", null, ResponseKind.JSON);
        add("ExaminationController", "getExaminationsSortedByUploadDate", "Ca khám", "Sắp xếp theo ngày tải lên",
                "Sắp xếp ca khám trong phạm vi được phép theo thời điểm tạo.", "200", "Trang ca khám đã sắp xếp", "EXAMINATION_PAGE", null, ResponseKind.JSON);
        add("ExaminationController", "getExaminationsFilteredByStudyDate", "Ca khám", "Lọc theo ngày chụp",
                "Lọc ca khám theo ngày studyDate định dạng yyyy-MM-dd.", "200", "Trang ca khám theo ngày chụp", "EXAMINATION_PAGE", null, ResponseKind.JSON);
        add("ExaminationController", "getExaminationsFilteredByUploadDate", "Ca khám", "Lọc theo ngày tải lên",
                "Lọc ca khám theo ngày dữ liệu được tạo trên hệ thống.", "200", "Trang ca khám theo ngày tải", "EXAMINATION_PAGE", null, ResponseKind.JSON);
        add("ExaminationController", "getExaminationById", "Ca khám", "Xem chi tiết ca khám",
                "Trả patient, DICOM instance và AI result của ca khám nếu người dùng có quyền sở hữu.", "200", "Chi tiết ca khám", "EXAMINATION", null, ResponseKind.JSON);
        add("ExaminationController", "getExaminationsByDoctorId", "Ca khám", "Lấy ca khám theo bác sĩ",
                "Bác sĩ chỉ xem danh sách của mình; trưởng khoa có thể xem theo bác sĩ bất kỳ.", "200", "Trang ca khám của bác sĩ", "EXAMINATION_PAGE", null, ResponseKind.JSON);
        add("ExaminationController", "getExaminationsByPatientId", "Ca khám", "Lấy ca khám theo bệnh nhân",
                "Trưởng khoa lấy lịch sử ca khám theo patient ID.", "200", "Trang ca khám của bệnh nhân", "EXAMINATION_PAGE", null, ResponseKind.JSON);
        add("ExaminationController", "getExaminationsByPatientIdAndStudyMonth", "Ca khám", "Lọc lịch sử khám theo tháng",
                "Lọc ca khám của bệnh nhân theo tháng chụp yyyy-MM.", "200", "Trang ca khám trong tháng", "EXAMINATION_PAGE", null, ResponseKind.JSON);
        add("ExaminationController", "getExaminationsByStatus", "Ca khám", "Lọc ca khám theo trạng thái",
                "Lọc theo trạng thái xử lý như NEED_VERIFY, VERIFIED hoặc REPORT_GENERATED.", "200", "Trang ca khám theo trạng thái", "EXAMINATION_PAGE", null, ResponseKind.JSON);
        add("ExaminationController", "getExaminationsByGrade", "Ca khám", "Lọc ca khám theo KL grade",
                "Lọc theo maxPredictedGrade từ 0 đến 4 trong phạm vi được phép.", "200", "Trang ca khám theo KL grade", "EXAMINATION_PAGE", null, ResponseKind.JSON);
        add("ExaminationController", "getPatientGradeStatistics", "Ca khám", "Thống kê bệnh nhân theo KL grade",
                "Đếm bệnh nhân theo kết quả KL mới nhất, hỗ trợ lọc khoảng ngày.", "200", "Số bệnh nhân theo từng grade", "GRADE_STATS", null, ResponseKind.JSON);
        add("ExaminationController", "markAsViewed", "Ca khám", "Đánh dấu đã xem",
                "Đánh dấu ca khám thuộc phạm vi người dùng là đã được mở xem.", "200", "Cập nhật thành công, không có response body", null, null, ResponseKind.VOID);
        add("ExaminationController", "getTotalExaminations", "Ca khám", "Đếm tổng ca khám theo người dùng",
                "Đếm ca khám theo userId; bác sĩ chỉ được hỏi dữ liệu của mình.", "200", "Tổng số ca khám", "LONG", null, ResponseKind.JSON);
        add("ExaminationController", "getTotalSevereExaminations", "Ca khám", "Đếm ca KL nặng",
                "Đếm ca có KL grade 3 hoặc 4 theo userId.", "200", "Số ca KL nặng", "LONG", null, ResponseKind.JSON);
        add("ExaminationController", "getTotalVerifiedExaminations", "Ca khám", "Đếm ca đã xác nhận",
                "Đếm ca có trạng thái VERIFIED theo userId.", "200", "Số ca đã xác nhận", "LONG", null, ResponseKind.JSON);
        ...
        add("ExaminationController", "getMyTotalVerifiedExaminations", "Ca khám", "Đếm ca đã xác nhận của tôi",
                "Đếm ca VERIFIED của người dùng đang đăng nhập.", "200", "Số ca đã xác nhận của tôi", "LONG", null, ResponseKind.JSON);
        add("ExaminationController", "getMyTotalUnverifiedExaminations", "Ca khám", "Đếm ca chưa xác nhận của tôi",
                "Đếm ca chưa VERIFIED của người dùng đang đăng nhập.", "200", "Số ca chưa xác nhận của tôi", "LONG", null, ResponseKind.JSON);
    }

    private void registerDicom() {
        addMultipart("DicomController", "uploadDicomFile", "DICOM", "Đọc metadata DICOM",
                "Tải một tệp DICOM để kiểm tra và trả các tag metadata; chưa tạo ca khám.", "200", "Danh sách tag DICOM", "DICOM_TAGS", ResponseKind.JSON);
        addMultipart("DicomController", "uploadBatch", "DICOM", "Tải nhiều tệp DICOM",
                "Kiểm tra nhiều tệp và tạo uploadSessionId để bác sĩ xác nhận trước khi lưu.", "200", "Kết quả tiếp nhận batch", "BATCH_UPLOAD", ResponseKind.JSON);
        addMultipart("DicomController", "uploadZipBatch", "DICOM", "Tải gói ZIP DICOM",
                "Giải nén, kiểm tra các tệp DICOM và tạo phiên xác nhận.", "200", "Kết quả tiếp nhận ZIP", "BATCH_UPLOAD", ResponseKind.JSON);
        add("DicomController", "getTotalStudies", "DICOM", "Đếm study DICOM",
                "Trả số StudyInstanceUID duy nhất trong phạm vi thống kê lâm sàng.", "200", "Tổng số study", "LONG", null, ResponseKind.JSON);
        addNotFound("DicomController", "getUploadSession", "DICOM", "Xem phiên tải DICOM",
                "Trả dữ liệu tạm của phiên upload nếu là người tải hoặc trưởng khoa.", "200", "Nội dung upload session", "UPLOAD_SESSION", null, ResponseKind.JSON);
        addNotFound("DicomController", "getInstanceImage", "DICOM", "Xem ảnh PNG của DICOM",
                "Trả ảnh PNG đã chuyển đổi của DICOM instance thuộc ca khám được phép.", "200", "Dữ liệu ảnh PNG", null, null, ResponseKind.PNG);
        addNotFound("DicomController", "getInstanceRaw", "DICOM", "Tải tệp DICOM gốc",
                "Trả tệp application/dicom gốc của instance thuộc ca khám được phép.", "200", "Dữ liệu tệp DICOM", null, null, ResponseKind.DICOM);
        add("DicomVerifyController", "verifyUploadSession", "Xác nhận DICOM", "Xác nhận phiên DICOM",
                "Lưu các bệnh nhân được chấp nhận và khởi chạy xử lý AI bất đồng bộ.", "200", "Trạng thái tiếp nhận xử lý", "VERIFY_MESSAGE", "DICOM_VERIFY_REQUEST", ResponseKind.JSON);
    }

    private void registerAiAndReviews() {
        add("AiController", "predictBatch", "Phân tích AI", "Chạy AI cho DICOM instances",
                "Gửi các DICOM instance đang chờ sang AI và trả kết quả theo ca khám.", "200", "Danh sách ca khám kèm AI result", "EXAMINATION_LIST", "AI_PREDICT_REQUEST", ResponseKind.JSON);
        addNotFound("AiController", "getHeatmapImage", "Phân tích AI", "Xem ảnh Grad-CAM",
                "Trả heatmap của AI result nếu người dùng có quyền với ca khám.", "200", "Dữ liệu ảnh heatmap", null, null, ResponseKind.PNG);
        addNotFound("AiController", "getImage", "Phân tích AI", "Xem ảnh lâm sàng",
                "Trả ảnh ROI, annotated hoặc ảnh liên quan theo image ID được phép.", "200", "Dữ liệu ảnh", null, null, ResponseKind.PNG);
        add("DiagnosisReviewController", "confirmAiGrade", "Xác nhận chẩn đoán", "Xác nhận kết quả AI",
                "Chọn kết quả AI làm KL grade cuối cùng cho một AI result.", "200", "Quyết định xác nhận", "DIAGNOSIS_REVIEW", null, ResponseKind.JSON);
        add("DiagnosisReviewController", "adjustKlGrade", "Xác nhận chẩn đoán", "Điều chỉnh KL grade",
                "Ghi KL grade bác sĩ chọn cùng ghi chú làm kết quả cuối cùng.", "200", "Quyết định điều chỉnh", "DIAGNOSIS_REVIEW", "ADJUST_KL_REQUEST", ResponseKind.JSON);
    }

    private void registerReports() {
        add("ReportController", "generatePdfReport", "Báo cáo PDF", "Tạo báo cáo PDF",
                "Tạo và lưu PDF từ kết quả cuối cùng của ca đã VERIFIED.", "200", "Metadata và URL báo cáo", "REPORT", null, ResponseKind.JSON);
        add("ReportController", "previewReport", "Báo cáo PDF", "Xem trước báo cáo PDF",
                "Trả PDF dạng inline để frontend hiển thị mà không ép tải xuống.", "200", "Nội dung PDF inline", null, null, ResponseKind.PDF);
        add("ReportController", "downloadReport", "Báo cáo PDF", "Tải báo cáo PDF",
                "Trả PDF dạng attachment để trình duyệt tải tệp về máy.", "200", "Nội dung PDF tải xuống", null, null, ResponseKind.PDF);
    }

    private void registerNotifications() {
        add("NotificationController", "getAllNotifications", "Thông báo", "Lấy toàn bộ thông báo của tôi",
                "Trả cả thông báo đã đọc và chưa đọc của tài khoản hiện tại, mới nhất trước.", "200", "Danh sách thông báo", "NOTIFICATION_LIST", null, ResponseKind.JSON);
    ...
    private void registerAccessManagement() {
        add("PermissionController", "getPermissionTree", "Phân quyền", "Lấy cây permission",
                "Trả feature và permission theo cấu trúc dùng cho màn hình phân quyền.", "200", "Cây feature/permission", "FEATURE_LIST", null, ResponseKind.JSON);
        add("PermissionController", "getRolePermissions", "Phân quyền", "Lấy permission của role",
                "Trả danh sách permission ID đang được gán cho role code.", "200", "Danh sách permission ID", "PERMISSION_IDS", null, ResponseKind.JSON);
        add("PermissionController", "updateRolePermissions", "Phân quyền", "Cập nhật permission của role",
                "Thay toàn bộ permission của role; không cho gán permission y tế cho ADMIN.", "200", "Cập nhật thành công, không có response body", null, "ROLE_PERMISSIONS_REQUEST", ResponseKind.VOID);
        add("PermissionController", "createPermission", "Phân quyền", "Tạo permission",
                "Tạo permission mới trong một feature, có thể phụ thuộc permission khác.", "201", "Permission đã tạo", "PERMISSION", "CREATE_PERMISSION_REQUEST", ResponseKind.JSON);
        add("PermissionController", "updatePermission", "Phân quyền", "Cập nhật permission",
                "Cập nhật code, tên, thứ tự, presentation và dependency của permission.", "200", "Permission sau cập nhật", "PERMISSION", "UPDATE_PERMISSION_REQUEST", ResponseKind.JSON);
        add("PermissionController", "deletePermission", "Phân quyền", "Xóa permission",
                "Gỡ quan hệ role/dependency rồi xóa permission theo ID.", "204", "Xóa thành công, không có response body", null, null, ResponseKind.VOID);
        add("FeatureController", "createFeature", "Tính năng", "Tạo feature",
                "Tạo nhóm chức năng dùng để tổ chức permission.", "201", "Feature đã tạo", "FEATURE", "FEATURE_REQUEST", ResponseKind.JSON);
        add("FeatureController", "updateFeature", "Tính năng", "Cập nhật feature",
                "Cập nhật tên và mô tả của nhóm chức năng.", "200", "Feature sau cập nhật", "FEATURE", "FEATURE_REQUEST", ResponseKind.JSON);
        add("FeatureController", "deleteFeature", "Tính năng", "Xóa feature",
                "Xóa feature cùng các permission và quan hệ phụ thuộc liên quan.", "204", "Xóa thành công, không có response body", null, null, ResponseKind.VOID);
    }

    private void registerSystemAndFiles() {
        add("AuditLogController", "getAuditLogs", "Nhật ký hệ thống", "Lấy audit log",
                "Admin xem nhật ký thao tác có phân trang, mới nhất trước.", "200", "Trang audit log", "AUDIT_PAGE", null, ResponseKind.JSON);
        addMultipart("FileUploadController", "uploadAvatar", "Tệp", "Tải tệp avatar",
                "Lưu ảnh avatar và trả URL nội bộ; endpoint không tự gán ảnh vào profile.", "200", "URL tệp đã lưu", "FILE_URL", ResponseKind.JSON);
        add("MailTestController", "sendTestEmail", "Kiểm thử email", "Gửi email kiểm thử",
                "Gửi email văn bản bằng mail server đang cấu hình; chỉ dùng để kiểm tra môi trường.", "200", "Kết quả gửi email", "STRING_SUCCESS", null, ResponseKind.TEXT);
        addMultipart("TestS3Controller", "testUpload", "Kiểm thử lưu trữ", "Kiểm thử upload S3",
                "Admin tải một tệp lên S3 để kiểm tra cấu hình storage.", "200", "URL do storage trả về", "S3_URL", ResponseKind.TEXT);
    }
