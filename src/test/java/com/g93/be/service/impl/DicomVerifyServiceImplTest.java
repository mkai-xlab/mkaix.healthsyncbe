...
public class DicomVerifyServiceImplTest {
    ...
    /**
     * Mục đích: Kiểm tra hành vi của hệ thống khi danh sách ID DicomInstance truyền
     * vào là null.
    ...
    /**
     * Mục đích: Kiểm tra luồng xử lý khi không tìm thấy thông tin User trong
     * Database.
    ...
    /**
     * Mục đích: Kiểm tra khả năng bắt lỗi khi Service AI (AiService) ném ra ngoại
     * lệ.
    ...
    /**
     * Mục đích: Kiểm tra luồng xử lý khi cả AiService và NotificationService đều bị
     * lỗi (chết dây chuyền).
     * Đầu vào: AiService ném ngoại lệ Timeout, NotificationService cũng ném ngoại
     * lệ khi cố gửi lỗi.
     * Hành động: Gọi processVerifiedSessionAsync().
    ...
    /**
     * Mục đích: Kiểm tra thông báo được gửi khi AI xử lý xong nhưng không trả về dự
     * đoán nào.
    ...
    /**
     * Mục đích: Kiểm tra logic gom nhóm dự đoán theo cùng một Bệnh nhân (Same
     * Patient).
     * Đầu vào: AiService trả về 2 kết quả thuộc về cùng 1 PatientId (lần lượt Grade
     * = 2 và Grade = 4).
     * Hành động: Gọi processVerifiedSessionAsync().
     ...
     * Đầu vào: AiService trả về 2 kết quả thuộc về 2 Bệnh nhân khác nhau (Grade 1
     * và Grade 4).
     * Hành động: Gọi processVerifiedSessionAsync().
    ...
    /**
     * Mục đích: Kiểm tra độ an toàn khi nhận được kết quả dự đoán thiếu thông tin
     * thiết yếu (Null Patient hoặc Null Grade).
